		...
		val html = createHtmlType => [
			head = createHeadType => [
				title = createTitleType => [
					it += (department.name ?: department.name)
				]
				meta += createMetaType => [
			...
			body = createBodyType => [
				for (programs : department.programs) {
					it += generate(programs)
				}
...
''' + department.programs.map[generateCheckFunction(it)].join
				]
			]
		]
		html
...
//   	}
	...
	def static void main(String[] args) throws IOException {
		...
		val html = new Plan2XhtmlGeneratorMain().generateHtml(quiz);
		if (args.length > 1) {
