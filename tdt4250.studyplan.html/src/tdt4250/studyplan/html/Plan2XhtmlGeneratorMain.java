...
public class Plan2XhtmlGeneratorMain {

		public static void main(String[] args) throws IOException {
			Department department = (args.length > 0 ? getQuiz(args[0]) : getSampleQuiz());
			String html = new Plan2XhtmlGeneratorMain().generateHtml(department);
			if (args.length > 1) {
				URI target = URI.createURI(args[1]);
				try (PrintStream ps = new PrintStream(depart.eResource().getResourceSet().getURIConverter().createOutputStream(target))) {
					ps.print(html);
				}
			} else {
